...
void cmsdasmetplotsexercise3(TString step) {
  ...
  TH1F *h_scale_raw =
      create1DEmptyhistoVarBins("h_scale_raw", bins, varbins, 1, 1);
  TH1F *h_resperp_raw_vspt =
      create1DEmptyhistoVarBins("h_resperp_raw_vspt", bins, varbins, 1, 1);
  TH1F *h_respara_raw_vspt =
      create1DEmptyhistoVarBins("h_respara_raw_vspt", bins, varbins, 1, 1);
  TH1F *h_resperp_raw_vspu = create1DEmptyhistoVarBins(
      "h_resperp_raw_vspu", bins_npv, varbins_npv, 1, 1);
  TH1F *h_respara_raw_vspu = create1DEmptyhistoVarBins(
      "h_respara_raw_vspu", bins_npv, varbins_npv, 1, 1);
  ...
  for (unsigned int i0 = 0; i0 < cuts.size(); ++i0) {
    ...
    TH1F *h_scale0_raw = create1Dhisto(
        t_, "1.", "ndimu>=2 && " + cuts[i0], "(upararaw)/dimupt", 4000, -100.,
        100., true, 2, 1, "h_scale0_raw", false, false);
    TH1F *h_upara_raw = create1Dhisto(t_, "1.", "ndimu>=2 && " + cuts[i0],
                                      "(upararaw-dimupt)", 100, -500., 500.,
                                      true, 2, 1, "h_upara_raw", false, false);
    TH1F *h_uperp_raw =
        create1Dhisto(t_, "1.", "ndimu>=2 && " + cuts[i0], "uperpraw", 20000,
                      -1000., 1000., true, 2, 1, "h_uperp_raw", false, false);
    ...
    h_scale_raw->SetBinContent(i0 + 1, 1. * (h_scale0_raw->GetMean()));
    h_scale_raw->SetBinError(i0 + 1, h_scale0_raw->GetMeanError());
    float scalecor_raw = 1.;
    if (h_scale0_raw->GetMean() != 0) {
      scalecor_raw = 1. / h_scale0_raw->GetMean();
    }
    if (step == "step3_resolution_pftype1") {
      //      h_respara_raw_vspt->SetBinContent(i0+1,scalecor_raw*(h_upara_raw->GetRMS()));
      //      h_respara_raw_vspt->SetBinError(i0+1,scalecor_raw*(h_upara_raw->GetRMSError()));
      //      h_resperp_raw_vspt->SetBinContent(i0+1,scalecor_raw*(h_uperp_raw->GetRMS()));
      //      h_resperp_raw_vspt->SetBinError(i0+1,scalecor_raw*(h_uperp_raw->GetRMSError()));
    }
  ...
  for (unsigned int i0 = 0; i0 < hist.size(); ++i0) {
    ...
    if (i0 == 0) {
      ...
      if (cName.Contains("perp")) {
        hist[i0]->GetYaxis()->SetRangeUser(0., 30.);
      }
